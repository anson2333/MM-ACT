...
def generate_action_with_step_tracking(
    ...
    prompt: (B, L0) tensor containing prompt token ids (without masked action tail).
    gen_length: number of tokens to generate (masked positions appended to prompt)
    block_length: size of each decoding block
    further_horizon: extra window length to include beyond block
    Returns DiffusionOutput with sequences, history (snapshots), first_unmask_step and nfe.
    """
    
    # ensure prompt is tensor on device
    if not torch.is_tensor(prompt):
        raise ValueError("prompt must be a torch.Tensor of token ids")
    ...
    eoa = 126099
    # 从后往前找第一个等于 eoa 的位置
    temp_new = (prompt == eoa)          # [B, L]
    # 每个样本里第一次出现的位置，没有则返回 length
    idx = temp_new.int().argmax(dim=1)  # [B]
    x = torch.full((B, L0), mask_id, dtype=torch.long, device=device)
    
    for b in range(B):
        end = idx[b] - gen_length          # 0-D tensor
        if end > 0:                        # 防止负数
            x[b, :end] = prompt[b, :end]
            x[b, idx[b]:] = prompt[b, idx[b]:]

    
    assert int(gen_length) % int(block_length) == 0, "gen_length must be divisible by block_length"
    num_blocks = int(gen_length) // int(block_length)
    base, rem = divmod(int(steps), num_blocks)
    ...
    for blk in range(num_blocks):
        s = idx[0] - gen_length + blk * block_length
        e = idx[0] - gen_length + (blk + 1) * block_length
        # TODO: 需要保证同一个prompt的两张图片，即两个子prompt，end of action的次序是一致的

        if cgws is not None:
            window_end = min(e + cgws, idx[0])
            window_slice = slice(s, window_end)
        else:
            window_slice = slice(s, idx[0])
        ...
        i = 1
        while True:
            # determine mask region for this iteration: within window, but only up to block_length
            if cgws is not None:
                mask_blk = (x[:, window_slice] == mask_id)
            else:
                mask_blk = (x[:, s:] == mask_id)
            # only consider positions within the block (first block_length positions of chunk)
            ...
            if use_cache:
                ...
                if cgws is not None:
                    ...
                    x0, tr_idx = get_transfer_index(
                        logits, temperature, target, mask_blk, x[:, window_slice], num_transfer[:, i], unmask_threshold
                    )
                    # write back to the full sequence
                    # convert boolean indices in window to global indices
                    x[:,window_slice][tr_idx] = x0[tr_idx]
                else:
                    logits = model(x[:, s:], past_key_values=pkv, use_cache=True).logits
                    x0, tr_idx = get_transfer_index(
                        logits, temperature, target, mask_blk, x[:, s:], num_transfer[:, i], unmask_threshold
                    )
    ...
    if config.experiment.function == "train":
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Rollout Progress")):
            ...
            for i in range(out.sequences.size(0)):
                ...
                eoa = 126099
                temp_new = (out.sequences[i, :] == eoa)
                idx  = temp_new.int().argmax(0)
                
                action_start_idx = idx - (config.training.chunk_size * batch_data["action_dims"][i])
                
                step_map_i = denoise_step_map_from_history(
                    out.history, mask_id=mask_id, sample_idx=i
                )
                action_step_map = step_map_i[action_start_idx:idx].tolist()
                
                result = {
                    "batch_idx": batch_idx,
                    "sample_idx": i,
                    "text": batch_data["texts"][i],
                    "generated_action_tokens": out.sequences[i, action_start_idx:idx].cpu().tolist(),
        ...
        output_file = os.path.join(
            output_dir, 
            f"rollout_outputs_epoch_{config.experiment.current_epoch}_{time_str}.json"
        )
